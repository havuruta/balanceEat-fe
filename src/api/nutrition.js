...
export const nutritionApi = {
  ...
  getAllNutritions: async (page = 0, size = 10) => {
    const response = await publicAxiosInstance.get(BASE_URL, {
      params: { page, size }
    })
  ...
  searchNutritions: async (name = null, category = null, page = 0, size = 10) => {
    ...
    const response = await publicAxiosInstance.get(`${BASE_URL}/search`, { params })
    return response.data
  },

  // ID로 영양 정보 조회
  getNutritionById: async (id) => {
    const response = await publicAxiosInstance.get(`${BASE_URL}/${id}`)
    return response.data
  }
} 